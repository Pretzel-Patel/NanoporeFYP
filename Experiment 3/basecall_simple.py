...
from step_0_configure import num_files, model, dorado_call_auto, input_fasta_prefix
...
for file_num in range(num_files):
    ...
    while i < len(rx_msg_all)-1:
        ...
        if reads_dict.get(read_id) != None:
            ...
            sys.exit()
        reads_dict[read_id] = rx_msg_all[i+1][:-1]
        i += 4
...
df.to_csv(f'4_basecalls_csv/all_reads_full_simple.csv')