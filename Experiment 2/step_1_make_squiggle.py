...
from step_0_configure import len_msg, PAD_str, num_trials, sd_min, sd_max, dwell_min, dwell_max, choice_list, mu, sigma, offset, scale, dorado_call_auto, kmer_len, kmer_centre, squiggle_generation, num_codebooks, pop_codeboooks, GC_codebooks, homo_codebooks
import os
import sys
os.chdir(sys.path[0])
...
option_labels = ['Testing popularity'] * pop_codeboooks + ['Testing GC content'] * GC_codebooks + ['Testing homopolymerity'] * homo_codebooks
...
for choice, sd_mean_scaled, dwell_mean in product(choice_list, range(sd_min,sd_max+1), range(dwell_min,dwell_max+1)):
    ...
    print("#"*90)
    print(f"Description: {description}.\nConducting {num_trials} trials with sd={sd_mean} and dwell={dwell_mean}")
    all_signals = []
    transmit_text = ""
...
print(f'Total time was {total_time:.1f} seconds')

if dorado_call_auto:
    import step_2_make_pod5_reads

global_time = time.time() - global_start_time
global_mins, global_seconds = int(global_time / 60), int(global_time % 60)
print(f"\nRun completed successfully\nTotal time for entire run was {global_mins} mins and {global_seconds} seconds")